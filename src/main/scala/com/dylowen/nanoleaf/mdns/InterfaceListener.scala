...
/**
  ...
  * @since Aug-2018
  */
private[mdns] object InterfaceListener extends LazyLogging {

  import NanoleafService._
  ...
  private case class ServiceEvent(event: jmdns.ServiceEvent) extends Message {
    def forParent: NanoleafService.ServiceEvent = NanoleafService.ServiceEvent(event)
  }

  private case class MDNSError(errorMessage: String) extends Message


  def actors(context: ActorContext[NanoleafService.Message]): Seq[ActorRef[Message]] = {
    getValidInterfaces
  ...
  private def interfaceActor(hostAddress: InetAddress,
                             ...
                             parent: ActorRef[NanoleafService.Message]): Behavior[Message] = {
    Behaviors.setup((setup: ActorContext[Message]) => {
      ...
      logger.debug(s"Registered ${NanoleafService.NanoleafServiceType} listener on $hostAddress")
      ...
      Behaviors.receive[Message]((_: ActorContext[Message], message: Message) => {
        ...
        case (_, PostStop) => {
          ...
          logger.debug(s"Removed ${NanoleafService.NanoleafServiceType} listener on $hostAddress")
          Behaviors.same
        }
